function getConfig(){
  if (window.KerbcycleQRGen && KerbcycleQRGen.nonce && KerbcycleQRGen.ajaxUrl) return KerbcycleQRGen;
  const el = document.getElementById('kc-qrgen-data');
  return {
    ajaxUrl: el ? el.getAttribute('data-ajax-url') : (window.ajaxurl || '/wp-admin/admin-ajax.php'),
    nonce:   el ? el.getAttribute('data-nonce') : ''
  };
}
...
(function($){
  ...
  $('#kc-generate-btn').on('click', function(){
    ...
    $.post(cfg.ajaxUrl, Object.assign({}, payload, { nonce: cfg.nonce })).done(function(resp){
